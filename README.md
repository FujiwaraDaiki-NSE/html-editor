...
起動スクリプトが3000番から空きポートを選び、Viteをそのポートで起動します。3000番ポートが使用中の場合は、次の空きポート（3001など）が選ばれます。同じPCでは `http://127.0.0.1:<表示されたポート>`、同じLANの別PCでは `http://<起動PCのIPアドレス>:<表示されたポート>` を開きます。ローカルAPIは別ポートへ公開せず、Vite経由で利用します。
