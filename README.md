...
- チャット欄の `@` または選択中の参照ボタンで要素を指し、注釈モードのドラッグで矩形だけを描いて、デッキへ保存せずAgentに送る
...
キーボード操作は画面左のキーボードボタンまたは `?` で一覧できます。主な操作は左右キーでスライド移動、Enter/F2で直接編集、チャット欄の `@` で要素を参照、`A` で矩形注釈モード、Escapeで取消、Cmd/Ctrl+ZでUndoです。
