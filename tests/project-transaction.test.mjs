...
test("restoring the pre-turn deck removes rejected slide HTML from active state", async () => {
  ...
  try {
    ...
    const rejected = {
      ...before,
      slides: before.slides.map((slide) => ({ ...slide, html: slide.html.replace('class="heading', 'class="heading content') })),
    };
...
test("discarding a rejected variation returns to main and removes its branch", async () => {
  ...
  try {
    ...
    await project.writeProject({
      ...before,
      slides: before.slides.map((slide) => ({ ...slide, html: slide.html.replace('class="heading', 'class="heading title') })),
    }, null, root);
    await assert.rejects(project.assertCommittable(root), (error) => error.code === "WEAVE_CONTENT_POLICY");
